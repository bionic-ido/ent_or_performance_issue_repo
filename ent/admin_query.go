...
func (aq *AdminQuery) QueryTeamMembers() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, admin.TeamMembersTable, admin.TeamMembersColumn),
		)
...
// QueryTeamLeader chains the current query on the "team_leader" edge.
func (aq *AdminQuery) QueryTeamLeader() *UserQuery {
	query := &UserQuery{config: aq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(admin.Table, admin.FieldID, selector),
			sqlgraph.To(user.Table, user.FieldID),
			sqlgraph.Edge(sqlgraph.O2O, false, admin.TeamLeaderTable, admin.TeamLeaderColumn),
		)
		fromU = sqlgraph.SetNeighbors(aq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (aq *AdminQuery) Clone() *AdminQuery {
	...
	return &AdminQuery{
		...
		withTeamMembers: aq.withTeamMembers.Clone(),
		withTeamLeader:  aq.withTeamLeader.Clone(),
...
func (aq *AdminQuery) WithTeamMembers(opts ...func(*UserQuery)) *AdminQuery {
	query := &UserQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withTeamMembers = query
	return aq
}

// WithTeamLeader tells the query-builder to eager-load the nodes that are connected to
// the "team_leader" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AdminQuery) WithTeamLeader(opts ...func(*UserQuery)) *AdminQuery {
	query := &UserQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withTeamLeader = query
	return aq
}
...
func (aq *AdminQuery) sqlAll(ctx context.Context) ([]*Admin, error) {
	var (
		...
		_spec       = aq.querySpec()
		loadedTypes = [2]bool{
			aq.withTeamMembers != nil,
			aq.withTeamLeader != nil,
		}
	)
	...
	if query := aq.withTeamMembers; query != nil {
	...
	if query := aq.withTeamLeader; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Admin)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
